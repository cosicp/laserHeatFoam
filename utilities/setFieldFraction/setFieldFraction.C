...
int main(int argc, char *argv[])
{
    ...
    argList::addOption("value", "word", "name of value to set (default: 1)");
...
#   include "createMesh.H"
    ...
    float value = args.getOrDefault("value", 1.0);
        ...
        if (cellZ >= zmax && cellZ <= layerHeight)
        {
            ...
            for (Foam::label subCellI = 0; subCellI < Foam::label(subCellCenters.size()); ++subCellI) {
                ...
                forAll(particleR, particleI)
                {
                    ...
                    if (distance <= particleR[particleI] && value==1)
                    {
                        alphaPhase1I[cellI] += value / subCellCenters.size();
                        break;
                    }else if (distance <=  particleR[particleI] && value != 1.0)
                    {
                        alphaPhase1I[cellI] = value;
                        break;
                    }
